...
def extract_asin(value: str) -> str:
    """Accept a 10-char ASIN or extract it from common Amazon URL patterns."""
    v = (value or "").strip()
    if not v:
        return ""
    if ASIN_RE.match(v):
    ...
    # Common patterns: /dp/ASIN, /gp/product/ASIN, /product/ASIN, /ASIN (in some short links)
    patterns = [
        r"/dp/([A-Z0-9]{10})",
    ...
    # Sometimes amazon short link contains ASIN as last path segment
    m = re.search(r"amazon\.[^/]+/.*/([A-Z0-9]{10})(?:[/?]|$)", v, flags=re.I)
    if m:
        return m.group(1).upper()
